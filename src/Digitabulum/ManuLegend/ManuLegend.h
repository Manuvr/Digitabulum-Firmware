...
* The ManuLegend is the filtering mechanism that we use to isolate glove data into what
*   the system that instantiates it needs. It is the logical interface between the core
*   sensor package and and any more-distant software that needs to know about the results
*   generated by the sensors.
*
* The ManuLegend is going to end up being a proximate driver of power managemnt. Many different
*   ManuLegends can exist at once for different client classes and systems. The boolean union of
*   all active ManuLegends directly translates to the minimum power draw on the glove. (Sensor
*   activity, radio use, etc).
*/
...
#define  LEGEND_MGR_FLAGS_LEGEND_STABLE      0x00000001   // If set, the map is not changing.
#define  LEGEND_MGR_FLAGS_LEGEND_SENT        0x00000002   // If set, the map is clear-to-send.
...
*   various subsystems in Digitabulum, and passes them off to any number of instantiated Legends.
*/
class LegendManager : public EventReceiver {
  public:
    LegendManager();
    ~LegendManager();
  ...
  private:
    ...
    /*
    * Accessors for legend stability.
    */
    inline bool _legend_is_stable() {         return (_class_flags & LEGEND_MGR_FLAGS_LEGEND_STABLE);  }
    inline void _legend_is_stable(bool en) {
      _class_flags = (en) ? (_class_flags | LEGEND_MGR_FLAGS_LEGEND_STABLE) : (_class_flags & ~(LEGEND_MGR_FLAGS_LEGEND_STABLE));
    }

    /*
    * Accessors for legend stability.
    */
    inline bool _legend_sent() {         return (_class_flags & LEGEND_MGR_FLAGS_LEGEND_SENT);  }
    inline void _legend_sent(bool en) {
      _class_flags = (en) ? (_class_flags | LEGEND_MGR_FLAGS_LEGEND_SENT) : (_class_flags & ~(LEGEND_MGR_FLAGS_LEGEND_SENT));
    }
